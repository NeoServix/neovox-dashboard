...
export default function Home() {
  ...
  const avanzarAPaso2 = () => {
    if (!agencyName || !contactEmail || !managerPassword || !confirmPassword) {
      return alert("Faltan datos para crear el perfil de gerencia.");
    }
    if (managerPassword !== confirmPassword) {
      return alert("Las contraseñas no coinciden. Revísalas antes de continuar.");
    }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="relative">
                  ...
                  <input 
                    ...
                    placeholder="********"
                    className="w-full p-4 rounded-xl border border-[#00A8E8]/20 bg-black/50 text-[#00A8E8] focus:border-[#00A8E8] outline-none transition-all text-sm pr-10"
                  />
                  <button 
                    type="button"
                    onClick={() => setShowPassword(!showPassword)}
                    className="absolute right-3 top-9 text-gray-500 hover:text-[#00A8E8] transition-colors"
                  >
                    {showPassword ? (
                ...
                <div>
                  <label className="block text-[10px] uppercase font-bold text-gray-500 mb-2 tracking-wider">Repetir Contraseña</label>
                  <input 
                    type={showPassword ? "text" : "password"} 
